...
public final class HuangTongChong {
    public static final String id = "huangTongChong";
    ...
    public static void skill1(GongData gongData, double multiplier) {
        ...
        RayTraceResult rayTraceResult = GetEntity.world.rayTrace(eyeLoc, direction, 20, FluidCollisionMode.NEVER,
                true, 0.5, it -> it.getScoreboardTags().contains("m"));

        Location hitPos;

        if (rayTraceResult == null) {
            hitPos = eyeLoc.clone().add(direction.multiply(20));
        } else {
        ...
        double damage = gongData.arrow * 30 * multiplier;
        ...
        SendInformation.sendActionBar(player, Component.text("§e[主动技]§6[烈弹]§a发动"));
