...
public final class FeiRen {
    public static final String id = "feiRen";
    ...
    public static void skill1(ZhanData zhanData, double multiplier) {
        ...
        //寻找终点
        RayTraceResult rayTraceResult = player.rayTraceBlocks(8, FluidCollisionMode.NEVER);
        Vector end;
        if (rayTraceResult != null) {
            end = rayTraceResult.getHitPosition();
        } else {
            end = eyeLoc.toVector().add(direction.clone().multiply(8));
        }
        ...
        ItemDisplay itemDisplay = DisPlayUtils.displayJian(eyeLoc, BiWeapon.FEIREN, new Vector3f(1.5f), distance, time, -0.55);
        Scheduler.syncLater(() -> {
