...
public final class Se4 {
    public static void spawn() {
        Location location = new Location(GetEntity.world, -698, 142, 351, 0, 0);
        Villager villager = GetEntity.world.spawn(location, Villager.class, false, it -> {
            ...
            it.customName(Component.translatable("pl.npc.name.se4"));
            it.setCustomNameVisible(true);

            List<MerchantRecipe> recipes = new ArrayList<>();

            // First book item (buy item)
            ...
            // Second book item (sell item)
            ItemStack sellItem = new ItemStack(Material.WRITTEN_BOOK, 1);
            sellItem.editMeta(meta -> {
                BookMeta bookMeta = (BookMeta) meta;
                bookMeta.title(Component.text("§9噩梦的结束"));
                bookMeta.author(Component.text("§9忘尘"));
                bookMeta.pages(List.of(
                        Component.translatable("pl.book.se4.page1"),
                        Component.translatable("pl.book.se4.page2")
                ));
            });
