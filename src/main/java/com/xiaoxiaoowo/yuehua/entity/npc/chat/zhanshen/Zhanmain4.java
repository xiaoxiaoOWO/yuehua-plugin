...
public final class Zhanmain4 {
    public static void spawn() {
        Location location = new Location(GetEntity.world, 3255, 20, -242, 0, 0);
        Villager villager = GetEntity.world.spawn(location, Villager.class, false, it -> {
            ...
            it.setAge(100000000);
            it.customName(Component.translatable("pl.npc.name.zhanmain4"));
            it.setCustomNameVisible(true);

            List<MerchantRecipe> recipes = new ArrayList<>();

            // First trade item (buy book about war preparations)
            ItemStack buyItem = new ItemStack(Material.WRITTEN_BOOK, 1);
            buyItem.editMeta(meta -> {
                BookMeta bookMeta = (BookMeta) meta;
                bookMeta.title(Component.text("§9战备补给"));
                bookMeta.author(Component.text("§9战神族主线"));
                bookMeta.pages(List.of(
                        Component.translatable("pl.book.zm2.page1")
                ));
            });

            // Sell item (book about medicine management)
