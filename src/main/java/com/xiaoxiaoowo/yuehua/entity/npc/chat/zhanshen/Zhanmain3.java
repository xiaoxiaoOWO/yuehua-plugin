...
public final class Zhanmain3 {
    public static void spawn() {
        Location location = new Location(GetEntity.world, 3299, 23, -253, 0, 0);
        Villager villager = GetEntity.world.spawn(location, Villager.class, false, it -> {
            ...
            it.setAge(100000000);
            it.customName(Component.translatable("pl.npc.name.zhanmain3"));
            it.setCustomNameVisible(true);

            List<MerchantRecipe> recipes = new ArrayList<>();

            // First trade item (buy red dye for book)
            ItemStack buyItem = RaceProvince.zhanShen.clone();

            // Sell item (book about war preparations)
