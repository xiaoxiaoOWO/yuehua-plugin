...
public final class Te4 {
    public static void spawn() {
        Location location = new Location(GetEntity.world, -105, 67, 46, 0, 0);
        Villager villager = GetEntity.world.spawn(location, Villager.class, false, it -> {
            ...
            it.customName(Component.translatable("pl.npc.name.te4"));
            it.setCustomNameVisible(true);

            List<MerchantRecipe> recipes = new ArrayList<>();

            // Pink Dye -> Written Book
            ItemStack buyItem = Other.NPC_PAOPAO.clone();

            ItemStack sellItem = new ItemStack(Material.WRITTEN_BOOK, 1);
            sellItem.editMeta(meta -> {
                BookMeta bookMeta = (BookMeta) meta;
                bookMeta.title(Component.text("§9热情的老板"));
                bookMeta.author(Component.text("§9小客栈老板"));
                bookMeta.pages(List.of(
                        Component.translatable("pl.book.te4.page1")
                ));
            });
